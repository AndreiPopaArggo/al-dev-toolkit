...
5. Report findings by severity — cite the `code` from `al_get_diagnostics` when a finding matches one, so the reader can jump straight to the Problems panel entry.
